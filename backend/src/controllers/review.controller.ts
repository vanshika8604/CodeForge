...
export async function review(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (error instanceof Error && (error.message === "EMPTY_AI_RESPONSE" || error.message === "INVALID_AI_RESPONSE")) {
      return res.status(502).json({ error: "AI review service returned an unexpected response" });
    }
    console.error(error);
